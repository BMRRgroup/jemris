...
RUN cd cvode-5.7.0; mkdir build; cd build; cmake -DSUNDIALS_PRECISION=${sundials_precision} -DENABLE_CUDA=ON -DEXAMPLES_ENABLE_CUDA=ON -DENABLE_MPI=ON -DENABLE_OPENMP=ON -DPTHREAD_ENABLE=ON -DBUILD_SHARED_LIBS=ON -DEXAMPLES_ENABLE=ON -DSUNDIALS_BUILD_WITH_PROFILING=ON -DENABLE_CUSPARSE=OFF ../; make; make install

# got issues with v6.4.1 even in serial jemris (likely sunctx issue)
