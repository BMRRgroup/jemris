...
* **Installation**:
   ...
   - A Dockerfile installing all prerequisites and compiling GPU-JEMRIS is provided in .devcontainer/. You can build an image using:
	
		 docker build -t <image_name> --build-arg CUDA_VERSION=12-8 --build-arg sundials_precision=double -f Dockerfile_GPU_JEMRIS .

	**Manual installation**:

   - JEMRIS prerequisites need to be met;
   ...
   - a newer version of CVode package should be installed for the GPU support.  
   	 GPU-JEMRIS can perform computations in **single OR double precision** which needs be specifed in the CVode installation:

 		
		 wget https://github.com/LLNL/sundials/releases/download/v6.4.1/cvode-6.4.1.tar.gz;
		 tar -xzvf cvode-6.4.1.tar.gz; cd cvode-6.4.1; mkdir build; cd build;
		 cmake -DSUNDIALS_PRECISION=double ../ -DENABLE_CUDA=ON -DENABLE_MPI=ON -DENABLE_OPENMP=ON;
		 make; make install;
   ...
   - GPU-JEMRIS compilation is separate from the CPU version (gjemris executable):
...
* **Tests for GPU-JEMRIS**:
   ...
   - **ctest** includes the following tests for the GPU simulator in both single and double precision:
		...
		2. Run GPU-simulations with a simple sample - if *jemris* (the CPU version) is installed and ctest ran for the CPU version, the new GPU signals are compared to the CPU signals.

		Update 2025: GPU signals might differ from CPU signals on different devices with small number of spins, so I increased the test tolearance to 10%.
